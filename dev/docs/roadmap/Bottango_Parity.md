...
| B10 | Exporting animations | Select characters/clips/assets/output profiles; validate dependencies; package `.anima` content; playback/trigger rules; deploy to headless runtime | document exporter + Anima Runtime | **Offline playback foundation shipped (backend):** the Python runtime loads `.character.anima` and executes `.scene.anima` v1 shows headless — clips, poses, waits, event gates, variables/branches, loops, deterministic parallel — streaming to any output adapter (`anima_studio/scene.py`; subset in `Scene_Format.md`). Studio-side export/deploy UI and audio/media packaging remain planned; depends on B01, B06, B07. |
