...
struct DocumentBuilder {
  ...
  std::vector<GBFaceRange> faces;
  std::vector<float> edgePoints;
  std::vector<GBEdgeRange> edges;
  ...
  GBDocument release(
    ...
    if (!nodes.empty()) {
      document.nodes = static_cast<GBAssemblyNode *>(
          std::calloc(nodes.size(), sizeof(GBAssemblyNode)));
      for (size_t index = 0; index < nodes.size(); ++index) {
        ...
        std::copy(nodes[index].transform, nodes[index].transform + 16,
...
GBDocument errorDocument(const std::string &message, double readSeconds = 0.0) {
  DocumentBuilder builder;
  ...
  try {
    ...
    if (!reader.Transfer(document)) {
      return errorDocument(
          "Open CASCADE Technology could not transfer the STEP Extended Data Exchange document",
          readSeconds);
    }
...
GBDocument gb_make_demo_document(
  ...
  DocumentBuilder builder;
  NodeBuilder root;
  root.name = "Kernel demo assembly";
...
const char *gb_occt_version(void) { return OCC_VERSION_COMPLETE; }
