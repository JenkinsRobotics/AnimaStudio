...
offending path, the same loader discipline as ``animacore.loader``.
...
``animacore.outputs.OutputAdapter`` protocol; ``parametric_feature``
(E2) contributions name a pure-data YAML template file inside the
bundle, parsed and validated by ``animacore.features`` — no Python
...
from animacore.features import (
    FeatureTemplate,
...
def _load_output_adapter_class(
    ...
    module_name = "animacore._animaext_" + re.sub(
...
def _load_feature_template(
    ...
    ``animacore.features.load_feature_template``, never imported or
