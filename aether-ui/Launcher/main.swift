...
// widget gallery (aether-ui/dist) over localhost and displays it, so
  ...
  WKNavigationDelegate
{
  ...
  private func startServer(repoRoot: URL) {
    let dist = repoRoot.appendingPathComponent("aether-ui/dist")
    let process = Process()
  ...
  func webView(
    ...
    if loadAttempts < 10 {
      ...
      webView.loadHTMLString(
        ...
        <p>Expected <code>aether-ui/dist</code> beside this app in the
        repository checkout — run
        <code>aether-ui/Launcher/build-launcher.sh</code> to rebuild.</p>
        </div></body>
        """, baseURL: nil)
    }
  }
